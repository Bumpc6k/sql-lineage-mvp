...
用法::
    ...
    .venv/bin/python scripts/ds_setup_demo.py                    # 全量重建 + 导出
    .venv/bin/python scripts/ds_setup_demo.py --no-export        # 只建数据
    .venv/bin/python scripts/ds_setup_demo.py --base-url http://localhost:12345/dolphinscheduler
...
from lineage.ds_client import (  # noqa: E402
    DsApiError,
    DsAuthError,
    DsClient,
    DsConnectionError,
...
def main(argv: Sequence[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        prog="python scripts/ds_setup_demo.py",
                ...
                for tdef in detail["taskDefinitionList"]:
                    ...
                    for i, script in enumerate(scripts, 1):
                        ...
                        (wf_dir / f"{tdef['name']}{suffix}.sql").write_text(
                            ...
                            f"，抽取自 {script['key']}\n{script['text']}\n", encoding="utf-8")
            manifest = {
                "note": "由 scripts/ds_setup_demo.py 从真实 DolphinScheduler 实例导出（GET /process-definition/{code}）",
