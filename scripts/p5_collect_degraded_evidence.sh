...
tmux new-session -d -s p5-legacy "cd $PROJ && .venv/bin/python scripts/p5_legacy_proxy.py --port $PORT >> $OUT/legacy_proxy.log 2>&1"
sleep 3
echo -n "   /analyze 探测: "
...
LINEAGE_SERVICE_URL="http://172.17.0.1:$PORT" LINEAGE_WF_NAME="wf_lineage_降级演示" \
  .venv/bin/python ds-plugin/verify_knowledge.py --degraded > "$OUT/degraded_log.txt" 2>&1
echo "   exit=$?"
