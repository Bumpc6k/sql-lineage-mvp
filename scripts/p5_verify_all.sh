...
bash ds-plugin/build.sh > "$OUT/build.log" 2>&1
echo "   build.sh exit=$?"
...
.venv/bin/python ds-plugin/verify_knowledge.py > "$OUT/sql_log.txt" 2>&1
echo "   exit=$?"
sed -n '/6) 断言检查/,$p' "$OUT/sql_log.txt"

echo
echo "########## 4) impact 模式回归（④ 段与下游影响仍在，⑤ 不出现） ##########"
.venv/bin/python ds-plugin/verify_impact_mode.py > "$OUT/impact_log.txt" 2>&1
echo "   exit=$?"
sed -n '/===*$/,$p' "$OUT/impact_log.txt" | tail -12

echo
echo "########## 5) 降级路径：旧版服务（无 /analyze）→ 回退 /parse ##########"
bash scripts/p5_collect_degraded_evidence.sh > "$OUT/degraded.txt" 2>&1
echo "   exit=$?"
sed -n '/6) 断言检查/,$p' "$OUT/degraded.txt"

echo
echo "########## 6) HTTP 端点回归 ##########"
bash scripts/p5_regression.sh > "$OUT/http.txt" 2>&1
echo "   exit=$?"
cat "$OUT/http.txt"
