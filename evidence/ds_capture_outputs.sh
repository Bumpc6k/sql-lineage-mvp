...
# 用法：bash evidence/ds_capture_outputs.sh [输出目录，默认 /tmp/ds_out]
set -u
cd "$(dirname "$0")/.."
OUT="${1:-/tmp/ds_out}"
mkdir -p "$OUT"
...
run setup       demos/ds_setup_demo.py
...
run export      demos/ds_export_table_graph.py ds_lineage.json /tmp/ds_table_graph.json
