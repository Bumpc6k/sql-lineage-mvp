...
用法示例::
    ...
    python -m lineage.cli generate sql --source ods.ods_卷烟产量流水 \
        --target cdw.dwd_卷烟产量明细 --metric 产量 --group-by plant_code --partition dt
    ...
    python -m lineage.cli generate apply --pipeline-file pipeline.json \
        --workflow-name wf_gen_产销存月报 --project-code 123 --apply --json
...
SUBCOMMANDS = ("scan", "upstream", "impact", "path", "cycle", "stats", "viz", "ds", "kb", "generate")
...
def cmd_generate_sql(argv: Sequence[str]) -> int:
    ...
    from .generate import format_sql_text, generate_sql
...
def cmd_generate_pipeline(argv: Sequence[str]) -> int:
    ...
    from .generate import format_pipeline_text, generate_pipeline
...
def cmd_generate_apply(argv: Sequence[str]) -> int:
    ...
    from .generate import apply_pipeline, format_apply_text, generate_pipeline
...
def cmd_generate_validate(argv: Sequence[str]) -> int:
    ...
    from .generate import format_validate_text, validate_generation
