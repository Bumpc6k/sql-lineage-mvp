...
from lineage.render.report import make_report_id, report_bases, reports_dir, save_report
from lineage_core.script_parser import detect_script_kind, looks_like_sql, parse_script
...
def _parse_tasks(task_items: Sequence[Dict[str, Any]], dialect: str,
    ...
    脚本按**类型分发**（``lineage.script_parser``）：

    * ``SQL`` 任务 / 纯 SQL 文本 → 直接丢给 :class:`lineage.parser.SqlLineageParser`；
    * ``SHELL`` / ``PYTHON`` 任务 → 先提取内嵌 SQL（``hive -e`` / ``beeline -f`` /
      heredoc / ``spark.sql(...)`` / ``pd.read_sql(...)`` …），再逐条解析。

    每个任务回填 ``script_kind`` / ``sql_count`` / ``unresolved_hints``，
    让「脚本里到底提取到几条 SQL、哪一段没法静态解析」在结果里看得见。

    其中 ``sql_count`` 的含义随脚本类型而定：SQL 任务 = 解析出的语句数；
    SHELL / PYTHON 任务 = 从脚本里**提取到的内嵌 SQL 片段数**。
    """
    ...
    for item in task_items:
        ...
        for script in extract_scripts(task):
            ...
            # SHELL / PYTHON 任务用**未清洗的 rawScript** 做提取：
            # ds_client.normalize_script 会把整段脚本截成第一条 `hive -e "..."` 的正文，
            # 后面的 beeline / heredoc / spark-sql 全被丢掉，拿它做工作流级分析会漏血缘。
            if entry["type"] in ("SHELL", "PYTHON") and raw_text.strip():
                source_text = raw_text
            else:
                source_text = text

            # --- 按脚本类型分发：SQL 直解；SHELL / PYTHON 先提取内嵌 SQL --- #
            kind = detect_script_kind(source_text, entry["type"])
            entry["script_kinds"].append(kind)
            statements: List[Dict[str, Any]] = []
            try:
                if kind == "sql":
                    statements = parser.parse_sql(source_text, source=entry["name"]) or []
                    entry["sql_count"] += len(statements)
                else:
                    extracted = parse_script(source_text, kind, dialect, task_type=entry["type"])
                    statements = extracted.get("statements") or []
                    entry["sql_count"] += int(extracted.get("sql_count") or 0)
                    entry["unresolved_hints"].extend(extracted.get("unresolved_hints") or [])
                    # 兜底：脚本里没有可识别的内嵌 SQL 命令，但整段文本本身就是 SQL
                    # （海豚 SHELL 任务的 rawScript 常被 ds_client 提前剥掉 hive -e 外壳）
                    if not statements and looks_like_sql(source_text):
                        statements = parser.parse_sql(source_text, source=entry["name"]) or []
                        entry["sql_count"] += len(statements)
            ...
            for st in statements:
                ...
                for col in st.get("column_lineage") or []:
                    ...
                    if st.get("source_hint"):
                        enriched["source_hint"] = st["source_hint"]
                    columns.append(enriched)
                entry["_columns"].extend(columns)
                statement = dict(st)
                statement["task"] = entry["name"]
                statement["task_type"] = entry["type"]
        ...
        entry["unresolved_hints"] = [
            h for h in entry["unresolved_hints"]
            if not (h in seen_hints or seen_hints.add(h))
        ]
        entry["script_kind"] = "+".join(sorted(set(entry["script_kinds"])))
        results.append(entry)
    return results
...
def _global_graph() -> Any:
    ...
    path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))),
...
def _knowledge(payload: Dict[str, Any], merged: Dict[str, Any],
    ...
    if not os.path.isabs(db_path):
        db_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))), db_path)
...
def analyze_workflow(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    workflow_info = {
        ...
        "sql_count": sum(t.get("sql_count", 0) for t in tasks),
        "script_kinds": {
            kind: sum(1 for t in tasks if t.get("script_kind") == kind)
            for kind in sorted({t.get("script_kind") or "" for t in tasks} - {""})
        },
        "unresolved_hint_count": sum(len(t.get("unresolved_hints") or []) for t in tasks),
        "chain": chain,
        ...
        "tasks": [
            {
                ...
                "code": task["code"],
                "parsed": task["parsed"],
                "script_kind": task.get("script_kind") or "",
                "sql_count": task.get("sql_count", 0),
                "unresolved_hints": task.get("unresolved_hints") or [],
