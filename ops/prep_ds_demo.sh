...
#
# 用法:
#   bash ops/prep_ds_demo.sh                # 常规：恢复插件 + 重建演示数据 + 端到端验证
#   bash ops/prep_ds_demo.sh --skip-verify  # 只恢复环境，不跑验证工作流
#   bash ops/prep_ds_demo.sh --skip-data    # 只恢复插件/配置，不动演示数据
#   bash ops/prep_ds_demo.sh --force-restart# 无条件重启海豚
#
# 完成后：
...
if [ ! -d "$ROOT/apps/ds-plugin" ]; then
  echo "❌ 无法定位项目目录（推导为 $ROOT，其中没有 apps/ds-plugin/）" >&2
  echo "   请用项目内路径运行：bash <项目>/ops/prep_ds_demo.sh" >&2
  exit 1
fi
# ⚠️ 唯一的编译/部署目录就是 <项目>/apps/ds-plugin/（内含源码、build.sh 与 jar）。
# 不要在别处保留第二份同名 jar —— 曾经因为副本较旧，本脚本按 md5 判断"不一致"
# 后把较旧的那份 cp 进容器，反而覆盖了新 jar，导致改动"看起来没生效"。
PLUGIN_DIR="$ROOT/apps/ds-plugin"
...
JAR_LOCAL="$PLUGIN_DIR/java/$JAR_NAME"
...
if [ ! -f "$JAR_LOCAL" ]; then
  if command -v javac >/dev/null 2>&1; then
    info "本地无 jar，执行编译（apps/ds-plugin/java/build.sh --no-restart）..."
    ( cd "$PLUGIN_DIR/java" && bash build.sh --no-restart ) >/tmp/prep_build.log 2>&1
    if [ -f "$JAR_LOCAL" ]; then
...
# ---------------------------------------------------------------------------
UI_CFG_SRC="$PLUGIN_DIR/frontend/conf/dynamic-task-type-config.yaml"
...
if [ -f "$UI_CFG_SRC" ]; then
  ...
  # 表单 JSON / 图标
  if [ -d "$PLUGIN_DIR/frontend/ui-static/lineage" ]; then
    docker exec "$CONTAINER" mkdir -p "$DS_HOME/ui/static/lineage" >/dev/null 2>&1
    for f in "$PLUGIN_DIR"/frontend/ui-static/lineage/*; do
      bn="$(basename "$f")"
  ...
  if [ -d "$PLUGIN_DIR/frontend/ui-static/assets" ]; then
    for f in "$PLUGIN_DIR"/frontend/ui-static/assets/*; do
      bn="$(basename "$f")"
      f_md5="$(md5sum "$f" | cut -d' ' -f1)"
      ...
      if [ -z "$c_md5" ]; then
        ...
        # 海豚的 nginx 开了 gzip_static：请求带 Accept-Encoding: gzip 时会优先返回同名 .gz。
        # 不更新/删掉 .gz 的话浏览器拿到的还是补丁前的旧 bundle（硬刷新也没用）。
        if docker exec "$CONTAINER" sh -c "rm -f $DS_HOME/ui/assets/$bn.gz && gzip -9 -n -k $DS_HOME/ui/assets/$bn && ls -l $DS_HOME/ui/assets/$bn.gz" >/dev/null 2>&1; then
          info "已刷新 gzip_static 副本: assets/$bn.gz"
        else
          docker exec "$CONTAINER" sh -c "rm -f $DS_HOME/ui/assets/$bn.gz" >/dev/null 2>&1 \
            && info "容器内没有 gzip，已删除旧 .gz 副本: assets/$bn.gz（改为直接下发明文）"
        fi
      else
        # .js 已经是最新，但 .gz 可能是上一轮补丁留下的旧内容（gzip_static 会优先命中它），
        # 这里解压比对一次，不一致就重建。
        gz_ok="$(docker exec "$CONTAINER" sh -c "zcat $DS_HOME/ui/assets/$bn.gz 2>/dev/null | md5sum | cut -d' ' -f1" 2>/dev/null || true)"
        if [ "$gz_ok" != "$f_md5" ]; then
          if docker exec "$CONTAINER" sh -c "rm -f $DS_HOME/ui/assets/$bn.gz && gzip -9 -n -k $DS_HOME/ui/assets/$bn" >/dev/null 2>&1; then
            warn "assets/$bn.gz 与 .js 不一致，已重建"
          else
            docker exec "$CONTAINER" sh -c "rm -f $DS_HOME/ui/assets/$bn.gz" >/dev/null 2>&1 \
              && warn "assets/$bn.gz 与 .js 不一致，已删除（改为直接下发明文）"
          fi
        else
...
if [ "$SKIP_DATA" = "1" ]; then
  ...
  if "$PY" "$ROOT/demos/ds_setup_demo.py" --no-export >/tmp/prep_demo_data.log 2>&1; then
    ok "烟草数仓演示数据已就绪"
...
if [ "$SKIP_VERIFY" = "1" ]; then
  ...
  if "$PY" "$PLUGIN_DIR/verify/verify_api.py" > /tmp/prep_verify.log 2>&1; then
  ...
  if "$PY" "$PLUGIN_DIR/verify/verify_dag.py" > /tmp/prep_verify_dag.log 2>&1; then
