...
"$PY" demos/ds_setup_demo.py --no-export 2>&1 | tail -12

echo "===== 2) LINEAGE_DAG 端到端（建 2 个演示工作流 + 给 wf_dws_汇总 追加节点） ====="
"$PY" apps/apps/ds-plugin/verify/verify_dag.py > /tmp/verify_dag.log 2>&1
...
bash evidence/collect_dag_evidence.sh > docs/p6_evidence/01_analyze_workflow_and_frontend.txt 2>&1
echo "collect exit=$?"
